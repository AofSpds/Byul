...
INTENT_TO_VIEWS: Mapping[str, Tuple[str, ...]] = {
    ...
    "pseries": ("PSERIES_REFERENCE_VIEW",),
    "raw": ("RAW_CORPUS",),
}
...
def _explicit_tags(text: str) -> Tuple[str, ...]:
    ...
    if "P-SERIES" in upper:
        tags.append("P_SERIES_REFERENCE")
    if "UNKNOWN" in upper:
        tags.append("UNKNOWN")
...
class MemoryCorpus:
    ...
    def content_digest(self) -> str:
        # Formatting-light digest: source + normalized non-empty line content.
        payload = "\n".join(
...
class Router:
    def route(
        ...
        validations = ["PROVENANCE_PRESERVATION", "SOURCE_DIGEST_CHECK"]
        notes: List[str] = []
        ...
        # P-series exact canonical rules are intentionally not bundled.
        pseries_state = "EXTERNAL_NOT_LOADED"
        notes.append("P-series PASS cannot be generated by v0.1")

        return RoutePlan(
            decision_state=decision_state,
            target_views=sorted(set(target)),
            required_validations=sorted(set(validations)),
            pseries_gate_state=pseries_state,
            notes=notes,
        )
